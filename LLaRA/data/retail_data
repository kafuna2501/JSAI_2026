...
class RetailData(data.Dataset):
    def __init__(
        ...
        self.padding_item_id = 793
        self.check_files()

        # cache all item ids for faster negative sampling
        # (exclude padding id if it ever appears)
        self.all_item_ids = [iid for iid in self.item_id2name.keys() if iid != self.padding_item_id]
    ...
    def __getitem__(self, i):
        ...
        cans_name = [self.item_id2name.get(can, f"<UNK_{can}>") for can in candidates]
        ...
        sample = {
            ...
            "len_cans": self.cans_num,  # fixed length guaranteed now
    ...
    def negative_sampling(self, seq_unpad, next_item):
        """
        Always returns exactly `self.cans_num` candidates including `next_item` once.
        If negatives are insufficient, fill with replacement (duplicates allowed).
        """
        seq_set = set(seq_unpad)

        # pool: items not in history and not the next item
        pool = [i for i in self.all_item_ids if (i not in seq_set) and (i != next_item)]
        need = self.cans_num - 1

        if need <= 0:
            return [next_item]

        if len(pool) >= need:
            negatives = random.sample(pool, need)
        elif len(pool) > 0:
            # fill with replacement to make fixed length
            negatives = pool + random.choices(pool, k=need - len(pool))
        else:
            # extreme fallback: allow negatives from all items except next_item
            fallback_pool = [i for i in self.all_item_ids if i != next_item]
            if len(fallback_pool) > 0:
                negatives = random.choices(fallback_pool, k=need)
            else:
                # degenerate case: only next_item exists
                negatives = [next_item] * need
